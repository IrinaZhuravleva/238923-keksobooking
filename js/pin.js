...
(function () {
  ...
  window.pins = {};
  window.pins.draw = function (pins) {
    ...
    for (var n = 0; n < pins.length; n++) {
      ...
      pinElement.style.left = pins[n].location.x + 'px';
      pinElement.style.top = pins[n].location.y + 'px';
      pinElement.innerHTML = '<img src="' + pins[n].author.avatar + '" class="rounded" width="40" height="40">';
    ...
    function disableAllPins() {
      var pinsElements = document.querySelectorAll('.pin');
      for (var index = 0; index < pinsElements.length; index++) {
        pinsElements[index].classList.toggle('pin--active', false);
      }
    }
